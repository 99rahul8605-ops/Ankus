...
async function getPointsBreakdown(userId) {
  const uid = String(userId);
  try {
    // Get current period start (or default to epoch)
    const rp = await ReferralPremium.findOne({ userId: uid });
    ...
    const points = Math.max(0, referrals - spent);
    return { referrals, spent, points };
  } catch (e) {
    return { referrals: 0, spent: 0, points: 0 };
  }
}
...
// ── Force Join Check ──────────────────────────────────────────────────────────
router.get("/force-join/check", async (req, res) => {
  try {
    const userId = getRequestUserId(req);
    ...
    for (const channelId of forceJoinChannels) {
      try {
        const r = await fetch(
          `https://api.telegram.org/bot${BOT_TOKEN_VAL}/getChatMember?chat_id=${encodeURIComponent(channelId)}&user_id=${userId}`,
        ...
        results.push({ channelId, joined, status: status || "unknown" });
      } catch {
        allJoined = false;
        results.push({ channelId, joined: false, status: "error" });
      }
    }

    // If all joined → trigger premium check for pending unverified referrals
    if (allJoined) {
      try {
        await fetch(`http://localhost:${process.env.PORT || 3000}/api/refer/verify-by-user`, {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ referredId: userId }),
        });
      } catch { /* non-fatal */ }
    }
