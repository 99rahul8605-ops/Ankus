...
router.post("/access/token/:userId", async (req, res) => {
  try {
    ...
    const expiresAt = new Date(Date.now() + 10 * 60 * 1000); // 10 min token
...
// ── Force Join (sirf Channel ID chahiye — naam/link auto Telegram se milta hai) ──
...
async function getChannelInfo(chatId, botToken) {
  ...
  try {
    ...
    // Auto-generate invite link from username if available
...
router.post("/force-join/check", async (req, res) => {
  try {
    ...
    const results = await Promise.all(ids.map(async (id) => {
      // Auto-fetch channel name, link, photo from Telegram
    ...
    // Referral bonus: referrer ko 8h access do jab referree saare channels join kare
    if (allJoined) {
      try {
