...
async function startBot() {
  ...
  bot.onText(/\/start(.*)/, async (msg, match) => {
    ...
    bot.sendMessage(chatId, welcomeText, { parse_mode:"HTML", reply_markup:{ inline_keyboard: startButtons } });
      ...
      console.error('/start handler error:', err.message, err.stack);
      bot.sendMessage(chatId, `⚠️ Something went wrong. Please try again in a moment! 🙏`).catch(() => {});
    }
  });
      ...
      try {
        ...
        if(autoLectureSession&&autoLectureSession.active){
          try{
            ...
            autoLectureSession.lectureCount=lNum; if(courseRoutes.saveAutoSession) await courseRoutes.saveAutoSession();
