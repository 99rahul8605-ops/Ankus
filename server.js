...
async function commitVideoLimitIncrement(userId) {
  const today = getTodayIST();
  let rec = await DailyVideoLimit.findOne({ userId });
  const newCount = (!rec || rec.resetDate !== today) ? 1 : rec.count + 1;
  await DailyVideoLimit.findOneAndUpdate({ userId }, { userId, count: newCount, resetDate: today }, { upsert: true });
...
// delivered_at is stored as a JSON string (chatId -> timestamp) — kept as-is
// from the original schema; Mongo has no atomic op for "set one key inside a
...
// Read-time guard for the 6h re-request cooldown — self-heals a stale entry
// (missing timestamp, expired window) by clearing it instead of blocking forever.
async function isDeliveryActive(fileRecordId, chatId, windowMs) {
  const doc = await FileRecord.findById(fileRecordId).select('delivered_to delivered_at').lean();
  if (!doc) return false;
  const arr = doc.delivered_to || [];
  if (!arr.includes(chatId)) return false;
  const at = JSON.parse(doc.delivered_at || '{}');
  const deliveredAt = at[chatId];
  if (!deliveredAt || (Date.now() - deliveredAt) >= windowMs) {
    await FileRecord.updateOne({ _id: fileRecordId }, { $pull: { delivered_to: chatId } });
    await stampMongoDeliveredAt(fileRecordId, chatId, null);
    return false;
  }
  return true;
}
...
app.get("/health", (req, res) => res.json({ status: "ok", uptime: process.uptime(), mongo: mongoose.connection.readyState===1?"connected":"disconnected" }));
...
async function creditValidReferralForBatch(botInstance, pending) {
  ...
  const batch = await Batch.findById(pending.batchId).select("name referralsRequired").lean();
  ...
  if (doc.validReferrals.length >= required) {
    ...
    doc.expiresAt = new Date(Date.now() + REFERRAL_PREMIUM_DURATION_MS);
    doc.validReferrals = [];
    await doc.save();
    setBatchPremiumCache(doc.userId, doc.batchId, true, doc.expiresAt);
    await botInstance.sendMessage(parseInt(pending.referrerId),
      `🎉 <b>Unlocked!</b>\n\n<b>${batch ? batch.name : "This batch"}</b> is now open for the next 7 days. 👑`,
      { parse_mode: "HTML" }
    ).catch(() => {});
  } else {
    await doc.save();
...
async function startBot() {
  ...
  bot.onText(/\/start(.*)/, async (msg, match) => {
    ...
    if (param) {
      if (param.startsWith("ref_")) {
        ...
        if (isValidReferrer && isNewUser && refBatchId) {
          try {
      ...
      if (param.startsWith("B")) {
        try {
          const batch = await BulkBatch.findOne({ batch_code: { $regex: new RegExp(`^${param}$`, "i") } }).lean();
      ...
      // Single file
      try {
        const record = await FileRecord.findOne({ code: { $regex: new RegExp(`^${param}$`, "i") } }).lean();
        ...
        if (isVideo && await isDeliveryActive(record._id, chatId, 6*60*60*1000)) return bot.sendMessage(chatId, `⚠️ This video was already delivered. You can request it again after 6 hours.`);
        if (isVideo && !isOwner(userId)) {
          ...
          const lim = await commitVideoLimitIncrement(userId);
          ...
          stampMongoDeliveredAt(record._id, chatId, Date.now());
          await scheduleUndeliver(record._id, record.code, chatId, new Date(Date.now()+6*60*60*1000));
  ...
  bot.onText(/\/delete (.+)/, async (msg,match) => {
    ...
    try {
      const delFile = await FileRecord.deleteOne({code:{$regex:new RegExp(`^${code}$`,"i")},uploaded_by:msg.from.id});
      if (delFile.deletedCount>0) return bot.sendMessage(chatId,`✅ File deleted!`);
      const delBatch = await BulkBatch.deleteOne({batch_code:{$regex:new RegExp(`^${code}$`,"i")},user_id:msg.from.id});
  ...
  bot.onText(TG_LINK_RE, (msg,match) => {
    ...
    enqueueFile(msg.from.id, async () => {
      ...
      try {
        ...
        await FileRecord.create({code,file_id:stored.file_id,file_type:stored.file_type,file_name:stored.file_name,uploaded_by:userId,expires_at:null,channel_msg_id:stored.channel_msg_id||null});
  ...
  // ── Force Join channel/group leave → revoke Premium + delete pending videos ──
  // Fires when a member leaves/is kicked from a chat where the bot is admin.
  // Ported from the older bot with one bug fixed: the "video deleted" notice used
  // to be sent unconditionally; now it only fires when a video was actually deleted.
  bot.on("chat_member", async (update) => {
    ...
    const isForceJoinChannel = allForceJoinChannels.some(c => c === chatUsername || c === chatIdStr || c === `-100${chatIdStr}`);
    if (!isForceJoinChannel) return;

    // Revoke Premium immediately if it was active.
    await revokeAllBatchPremiumForUser(userId).catch(() => {});

    // Delete any videos still pending auto-delete in this user's DM — but only
    // notify them if we actually deleted something (this is the bug fix).
    ...
    if (pendingVideos.length > 0) {
      ...
      try { await bot.sendMessage(userId, "⚠️ You left a required channel/group, so your premium access and pending videos have been removed."); } catch (_) {}
    }
  });
