...
let addWords = [];     // words appended to file names (before extension)
let replaceWords = []; // [{from, to}] pairs — find-and-replace in file names
...
function cleanFileName(name) {
  ...
  // Apply replaceWords — same unicode-safe indexOf approach
  ...
  // Collapse leftover separators (_, space, -, ., :) into single _
  result = result
    .replace(/[_ .\-:]{2,}/g, "_")
    .replace(/^[_ .\-:]+|[_ .\-:]+$/g, "")
    .trim();
  // Append addWords before extension
  if (addWords.length) {
...
async function startBot() {
  // Drain all pending updates: fetch last update_id then advance offset past it.
  // Without this, startPolling() replays the same /start (or any other) message
  // from the previous session, causing duplicate responses.
  try {
    ...
    if (drainData.ok && drainData.result && drainData.result.length > 0) {
      const lastId = drainData.result[drainData.result.length - 1].update_id;
      // Advance Telegram's server-side offset so these updates are never re-delivered
      await fetch(
        `https://api.telegram.org/bot${TOKEN}/getUpdates?offset=${lastId + 1}&timeout=0`,
        { signal: AbortSignal.timeout(10000) }
      );
      console.log(`Drained pending updates up to update_id ${lastId}`);
    } else {
      console.log("No pending updates to drain.");
    }
  } catch (err) {
    console.warn("Could not drain updates (continuing):", err.message);
  }
  ...
  const _botStartTime = Math.floor(Date.now() / 1000); // Unix seconds, for stale-update rejection
  bot.onText(/\/start(.*)/, async (msg, match) => {
    if (isGroupChat(msg)) return; // Ignore all group messages
    // Reject stale updates replayed after bot restart
    if (msg.date < _botStartTime) return;
    // Deduplicate genuine Telegram duplicate deliveries
    const _msgKey = `${msg.chat.id}:${msg.message_id}`;
    if (_startSeen.has(_msgKey)) return;
    _startSeen.add(_msgKey);
    setTimeout(() => _startSeen.delete(_msgKey), 60000);
          ...
          try {
            ...
            if (recordData.isNew) {
              ...
              bot.sendMessage(parseInt(referrerId),
                `🎉 <b>New Referral!</b>\n\n✦ ${firstName}${lastName} joined using your link!\n\n✅ <b>+24h Access Activated!</b>\n⭐ <b>+1 Point earned!</b>\n🦋 Total Points: <b>${stats.points}</b>`,
                { parse_mode: 'HTML' }
              ).catch(() => {});
            }
          } catch (_) {}
    ...
    const welcomeText = isOwner(userId)
      ? `🦋 Hello Admin!\n\n✅ Browse all lectures below.\n\n` +
        ...
        `✦ /cancel — cancel bulk\n\n` +
        `📝 <b>File Name Tools</b>\n` +
        `✦ /rmword 'word' — strip word from names\n` +
        `✦ /addword 'word' — append word to names\n` +
        `✦ /replaceword 'old' 'new' — replace word\n\n` +
        `📡 <b>Broadcast</b>\n` +
        `✦ /broadcast <text>\n` +
        `✦ /broadcast --pin <text>\n` +
      ...
      : `🦋 Hello ${msg.from.first_name}!\n\n✅ Tap below to browse all lectures.\n\n` +
        `❄️ <b>Get Free Access:</b>\n` +
        `✦ ➜ <b>Watch Ads</b> — <b>+8h</b> each (max 3× / day)\n` +
        `✦ ➜ <b>Refer a Friend</b> — <b>You get 24h + 1 point,</b> they get 14h` +
        (referLink ? `\n\n🔗 <b>Your Link:</b>\n<code>${referLink}</code>` : '');

    bot.sendMessage(chatId, welcomeText, {
      parse_mode: isOwner(userId) ? undefined : 'HTML',
      reply_markup: {
        inline_keyboard: [[
  ...
  // ── /addword — manage words appended to file names on save ─────────────────────
  //   /addword 'word'         — append word to all new file names
  //   /addword remove 'word'  — remove from append list
  //   /addword list           — show current list
  //   /addword clear          — clear all
  bot.onText(/\/addword(.*)/, async (msg, match) => {
    ...
    // Supports full phrases/sentences — original casing + spaces are preserved
    ...
    if (!phrase) {
      return bot.sendMessage(chatId,
        ...
        `<i>Example: /addword 'CA Inter | FocusX'</i>`,
        { parse_mode: "HTML" });
    }
    ...
    addWords.push(phrase); // preserves original casing and spaces
    return bot.sendMessage(chatId,
  ...
  //   /replaceword remove 'old'  — remove rule
  //   /replaceword list           — show all rules
  //   /replaceword clear          — clear all
...
• DB: ${require('mongoose').connection.readyState === 1 ? '🟢 Connected' : '🔴 Disconnected'}`;
  ...
  bot.onText(/\/addadmin(?:\s+(.+))?/, async (msg) => {
    if (isGroupChat(msg)) return;
    const userId = msg.from?.id;
    if (userId !== OWNER_ID) return; // Owner only (not admins)
    const chatId = msg.chat.id;
    const parts = msg.text.trim().split(/\s+/);
    const targetId = parts[1];
    if (!targetId || isNaN(parseInt(targetId))) {
      return bot.sendMessage(chatId, `⚠️ Usage: /addadmin <user_id>\n\nGet a user\'s ID using @userinfobot`);
    }
    const targetStr = String(parseInt(targetId));
    if (parseInt(targetStr) === OWNER_ID) {
      return bot.sendMessage(chatId, `⚠️ Owner is already the super-admin.`);
    }
    try {
      await Admin.findOneAndUpdate(
        { adminId: targetStr },
        { adminId: targetStr, addedBy: String(OWNER_ID) },
        { upsert: true, new: true }
      );
      adminSet.add(targetStr);
      bot.sendMessage(chatId, `✅ User <code>${targetStr}</code> is now an admin.\n\nThey can now use all admin commands and give access to users.`, { parse_mode: 'HTML' });
    } catch (e) {
  ...
  bot.onText(/\/removeadmin(?:\s+(.+))?/, async (msg) => {
    if (isGroupChat(msg)) return;
    const userId = msg.from?.id;
    if (userId !== OWNER_ID) return; // Owner only
    const chatId = msg.chat.id;
    const parts = msg.text.trim().split(/\s+/);
    const targetId = parts[1];
    if (!targetId || isNaN(parseInt(targetId))) {
      return bot.sendMessage(chatId, `⚠️ Usage: /removeadmin <user_id>`);
    }
    const targetStr = String(parseInt(targetId));
    try {
      const res = await Admin.deleteOne({ adminId: targetStr });
      adminSet.delete(targetStr);
      if (res.deletedCount === 0) {
        return bot.sendMessage(chatId, `⚠️ User <code>${targetStr}</code> is not an admin.`, { parse_mode: 'HTML' });
      }
      bot.sendMessage(chatId, `✅ User <code>${targetStr}</code> removed from admins.`, { parse_mode: 'HTML' });
    } catch (e) {
      bot.sendMessage(chatId, `❌ Error: ${e.message}`);
    }
  });

  // ── /admins — list all admins (Owner + Admins) ────────────────────────────────
  bot.onText(/\/admins$/, async (msg) => {
    ...
    try {
      ...
      if (!admins.length) {
        return bot.sendMessage(chatId, `👑 <b>Admin List</b>\n\nNo extra admins set.\nUse /addadmin &lt;user_id&gt; to add one.`, { parse_mode: 'HTML' });
      }
      const lines = admins.map((a, i) =>
        `${i + 1}. <code>${a.adminId}</code> — added ${a.addedAt.toLocaleDateString('en-IN')}`
      ).join('\n');
      bot.sendMessage(chatId, `👑 <b>Admin List</b> (${admins.length})\n\n${lines}\n\n<i>Use /addadmin or /removeadmin to manage.</i>`, { parse_mode: 'HTML' });
    } catch (e) {
      bot.sendMessage(chatId, `❌ Error: ${e.message}`);
    }
  });
  ...
  bot.onText(/\/giveaccess(?:\s+(.+))?/, async (msg) => {
    ...
    try {
      const r = await fetch(`http://localhost:${PORT}/api/access/grant`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ userId: String(parseInt(targetId)), hours }),
      });
      const data = await r.json();
      if (!data.success) return bot.sendMessage(chatId, `❌ Failed: ${data.error || 'Unknown error'}`);
      const until = new Date(data.expiresAt).toLocaleString('en-IN', { timeZone: 'Asia/Kolkata', hour12: true });
      bot.sendMessage(chatId,
        `✅ <b>Access Granted!</b>\n\n` +
