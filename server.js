...
async function startBot() {
  ...
  // Delete webhook + drop pending to prevent 409 Conflict on Render redeploy
  try {
    await bot.deleteWebhook({ drop_pending_updates: true });
  } catch (e) {
    console.warn("deleteWebhook skipped:", e.message);
  }
...
• DB: ${require('mongoose').connection.readyState === 1 ? '🟢 Connected' : '🔴 Disconnected'}`;
  ...
  bot.on('chat_member', async (update) => {
    ...
    // Notify user only if there were videos to delete
    if (msgs && msgs.length > 0) {
      try {
        await bot.sendMessage(userId, 'You left a required channel/group — your videos have been deleted.');
      } catch (_) {}
    }
  });
