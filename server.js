...
function cleanFileName(name) {
  ...
  if (addWords.length) {
    result = result ? result + "_" + addWords.join("_") : addWords.join("_");
  }
...
async function startBot() {
  ...
  bot.onText(/\/addword(.*)/, async (msg, match) => {
    ...
    if (arg.toLowerCase() === "clear") {
      ...
      addWords = [];
      return bot.sendMessage(chatId, `🗑️ Cleared ${count} word(s) from append list.`);
    }

    const removeMatch = arg.match(/^remove\s+['"]?(.+?)['"]?$/i);
    if (removeMatch) {
      const word = removeMatch[1].trim().toLowerCase();
      const before = addWords.length;
      addWords = addWords.filter(w => w.toLowerCase() !== word);
      if (addWords.length < before)
        return bot.sendMessage(chatId, `✅ Removed <code>${esc(word)}</code> from append list.`, { parse_mode: "HTML" });
      return bot.sendMessage(chatId, `⚠️ Word not found in append list.`);
    }
    ...
    if (!word) {
      return bot.sendMessage(chatId,
        ...
        `• <code>/addword 'word'</code> — append word to file names\n` +
        `• <code>/addword remove 'word'</code> — remove from list\n` +
        `• <code>/addword list</code> — show current list\n` +
        `• <code>/addword clear</code> — clear all`,
        { parse_mode: "HTML" });
    }

    if (addWords.includes(word.toLowerCase())) {
      return bot.sendMessage(chatId, `⚠️ <code>${esc(word)}</code> is already in the list.`, { parse_mode: "HTML" });
    }
    addWords.push(word.toLowerCase());
    return bot.sendMessage(chatId,
      `✅ Added <code>${esc(word)}</code> to append list.\n📋 Total: ${addWords.length}\n\n<i>Appended to all file names on save.</i>`,
      { parse_mode: "HTML" });
  });
