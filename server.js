...
async function sendFile(bot, chatId, record) {
  ...
  // If the stored file_id is actually an HTTP URL (old/bad data), skip directly to
  // the channel-forward fallback rather than sending an invalid identifier to Telegram.
  const fileIdIsUrl = typeof record.file_id === 'string' && record.file_id.startsWith('http');

  if (!fileIdIsUrl) {
    // Normal path: send by file_id
    try {
      ...
      // file_id invalid (bot changed) — fall through to channel-forward below
      if (!STORAGE_CHANNEL_ID || !record.channel_msg_id) throw err;
    }
  }

  // Fallback: forward from storage channel (covers bot-change AND bad file_id data)
  ...
  throw new Error(`Cannot deliver file "${record.file_name}" — file_id is invalid and no channel backup available. Please re-upload this file.`);
}
...
async function startBot() {
  ...
  bot.onText(/\/start(.*)/, async (msg, match) => {
    ...
    bot.sendMessage(chatId, welcomeText, {
      ...
      bot.sendMessage(chatId, `⚠️ Something went wrong. Please try again! 🙏`).catch(() => {});
    }
  });
